...
describe Dragnet::Survey::Naming do
  ...
  describe '#author_id' do
    ...
    context 'when survey has an author_id' do
      let(:survey) { Dragnet::Survey.new(author_id: author_id) }
      let(:author_id) { Dragnet::UUID.generate }
    ...
    context 'when the number of auto named surveys is non-zero' do
      before do
        (1..3).to_a.sample.times do
          Dragnet::Survey.create!(author: author)
        end
      end
